document.getElementById('btn_agregar').addEventListener('click', function(event) {
    ...
    if (descuento === "" || isNaN(descuento) || descuento <= 0) {
        document.getElementById('errorDescuento').innerText = "El descuento debe ser un número válido";
        isValid = false;
    }
    if (descripcion === "" ) {
        document.getElementById('errorDescripcion').innerText = "Ingrese una descripción";
        isValid = false;
    }
    if (sProducto === "") {
        document.getElementById('errorsProduct').innerText = "Ingrese un producto";
        isValid = false;
    }
    ...
    if (!estadoOfertaActivo && !estadoOfertaInactivo) {
        document.getElementById('errorEstadoOferta').innerText = "Seleccione el estado de la oferta.";
        isValid = false;
    }
    if (isValid) {
...
document.getElementById("existencia").addEventListener("keypress", function(event) {
